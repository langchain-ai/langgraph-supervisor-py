...
from langgraph_supervisor.message import (
    add_xml_tags_to_message_content,
    remove_xml_tags_from_message_content,
)
...
def test_process_input_message():
    ...
    result = add_xml_tags_to_message_content(human_message)
    ...
    result = add_xml_tags_to_message_content(ai_message)
...
def test_process_input_message_content_blocks():
    content_blocks = [
...
def test_process_output_message():
    # Test that non-AI messages are returned unchanged.
    human_message = HumanMessage(content="Hello")
    result = remove_xml_tags_from_message_content(human_message)
    assert result == human_message

    # Test that messages with empty content are returned unchanged.
    ai_message = AIMessage(content="", name="assistant")
    result = remove_xml_tags_from_message_content(ai_message)
    assert result == ai_message
